...
async function copyAllTranslationKeys() {
  ...
  console.log(
    `Starting to process ${totalElements} elements in the main content...`,
  )
  ...
  for (let i = 0; i < totalElements; i++) {
    ...
    if (i % 100 === 0 || i === totalElements - 1) {
      console.log(
        `Processed ${i + 1}/${totalElements} elements. Found ${
          keys.length
        } unique keys so far.`,
      )
    }
  ...
  if (keys.length > 0) {
    const sortedGroupedKeys = Object.fromEntries(
      Object.entries(groupedKeys).sort(([a], [b]) => a.localeCompare(b)),
    )

    const keysText = Object.entries(sortedGroupedKeys)
      .map(
        ([category, categoryKeys]) =>
          `${category}:\n${categoryKeys.sort().join('\n')}`,
      )
      .join('\n\n')
